    ...
    <>
      <form className='m-auto my-16 max-w-[500px] bg-white p-8 border-2 shadow-md rounded-md'>
        {loading ? (
          ...
          <>
            <h2 className='text-center text-2xl mb-6'>Hello there! Your login awaits</h2>
            <div className="mb-6">
              <label htmlFor="email" className="after:content-['*'] after:ml-0.5 after:text-red-500">Email</label>
              <Input type="text" name="email" id="email" value={formData.email} placeholder="user123@email.com" onChange={handleChange} />
              {fieldError("email")}
            </div>
            ...
            <div className='pt-4'>
              <Link to="/signup" className='text-blue-500'>Looking to join? Sign up here</Link>
            </div>
          </>
        )}
      </form>
